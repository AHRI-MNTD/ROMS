...
export default function LabWorkflowPage() {
  const { data, isLoading, error } = useQuery({
    queryKey: ["lab-workflow-list"],
    queryFn: async () => {
      try {
        const resp = await apiClient.get("/domains/lab-workflow/protocols");
    ...
    <div style={{ padding: "24px 28px", maxWidth: 1100 }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: "var(--fs-sm)", color: "var(--color-text-muted)" }}>
          Manage lab workflow & experiments records. Showing live data from the ROMS API.
        </p>
      </div>
        ...
        <>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", background: "var(--color-surface-2)", border: "1px solid var(--color-border)", borderRadius: "var(--radius)" }}>
              ...
              <th style={{padding:'5px 10px',fontSize:'var(--fs-xs)',fontWeight:700,letterSpacing:'0.07em',textTransform:'uppercase',color:'var(--color-text-faint)',textAlign:'left'}}>{'VERSION'.replace('_',' ')}</th>
              ...
              <td style={{padding:'5px 10px',fontSize:'var(--fs-xs)',color:'var(--color-text-muted)'}}>{String((row as Record<string,unknown>)['version'] ?? '—')}</td>
