...
export default function InfrastructurePage() {
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        const resp = await apiClient.get("/domains/infrastructure/integration-jobs");
    ...
    <div style={{ padding: "24px 28px", maxWidth: 1100 }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: "var(--fs-sm)", color: "var(--color-text-muted)" }}>
          Manage infrastructure & it services records. Showing live data from the ROMS API.
        </p>
      </div>
        ...
        <>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", background: "var(--color-surface-2)", border: "1px solid var(--color-border)", borderRadius: "var(--radius)" }}>
              ...
              <th style={{padding:'5px 10px',fontSize:'var(--fs-xs)',fontWeight:700,letterSpacing:'0.07em',textTransform:'uppercase',color:'var(--color-text-faint)',textAlign:'left'}}>{'SYSTEM'.replace('_',' ')}</th>
              <th style={{padding:'5px 10px',fontSize:'var(--fs-xs)',fontWeight:700,letterSpacing:'0.07em',textTransform:'uppercase',color:'var(--color-text-faint)',textAlign:'left'}}>{'DIRECTION'.replace('_',' ')}</th>
              ...
              <th style={{padding:'5px 10px',fontSize:'var(--fs-xs)',fontWeight:700,letterSpacing:'0.07em',textTransform:'uppercase',color:'var(--color-text-faint)',textAlign:'left'}}>{'RANAT'.replace('_',' ')}</th>
                </tr>
              </thead>
              <tbody>
                {data.data.map((row, i) => (
                  ...
                  >
              <td style={{padding:'5px 10px',fontSize:'var(--fs-xs)',color:'var(--color-text-muted)'}}>{String((row as Record<string,unknown>)['system'] ?? '—')}</td>
              <td style={{padding:'5px 10px',fontSize:'var(--fs-xs)',color:'var(--color-text-muted)'}}>{String((row as Record<string,unknown>)['direction'] ?? '—')}</td>
              <td style={{padding:'5px 10px',fontSize:'var(--fs-xs)',color:'var(--color-text-muted)'}}>{String((row as Record<string,unknown>)['status'] ?? '—')}</td>
              <td style={{padding:'5px 10px',fontSize:'var(--fs-xs)',color:'var(--color-text-muted)'}}>{String((row as Record<string,unknown>)['ranAt'] ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
