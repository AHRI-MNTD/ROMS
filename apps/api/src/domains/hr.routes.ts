...
import { CreateStaffProfileSchema } from "@roms/shared";
...
router.get("/staff", requireAuth, requirePermission("hr:read"), async (req, res) => {
  ...
  const [data, total] = await Promise.all([
    prisma.staffProfile.findMany({ skip: (page - 1) * 20, take: 20, include: { user: { select: { displayName: true, email: true, roles: true } } }, orderBy: { department: "asc" } }),
